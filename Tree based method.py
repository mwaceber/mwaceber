...
clf.fit(X, High)
# max_depth (how deep to grow the tree), min_sample_split (min no. of observations in a node to be eligible for splitting)
# criterion (whether to use Gini or cross entropy as the split criterion)
ax = subplots(figsize = (12, 12))[1]
plot_tree(clf, feature_names= feature_name, ax= ax)
export_text(clf, feature_names= feature_name, show_weights= True)
# Estimate test error by splitting the observation to training and test datasets
validation = skm.ShuffleSplit(n_splits= 1, test_size= 200, random_state= 0)
results = skm.cross_validate(clf, D, High, cv = validation)
results['test_score']
# Next is prune the tree for better classification performance
(X_train, X_test, High_train, High_test) = skm.train_test_split(X, High, test_size= 0.5, random_state = 0)
# Refit the whole tree on the training set and then extract cost complexity values
...
plot_tree(best_, feature_names= feature_name, ax = ax)
...
feature_imp = pd.DataFrame({'importance': bag_boston.feature_importances_}, index = feature_names)
